...
### Fixed
- The split **EM Segmentation** and **FM Segmentation** widgets no longer crash when no mitochondria are found; they now show an error. The segmentation code raises a `NoSegmentationError` instead of returning the string `'No segmentation'`, which every caller had to remember to check for (#25 fixed this only in Run Registration) ([#50](https://github.com/martlj/napari-clemreg/issues/50), [PR #71](https://github.com/martlj/napari-clemreg/pull/71)).
- **Run Registration no longer hangs silently when EM segmentation fails.** Segment-Flow's errors (Nextflow or Conda missing, or the pipeline failing) subclassed `RuntimeError`, which the background workers behind **Register** swallow without reporting an error or finishing. They now subclass a new `ClemregError(Exception)`, and any other `RuntimeError` in Register's segmentation or registration steps is re-raised as a `ClemregError`, so it's reported ([#53](https://github.com/martlj/napari-clemreg/issues/53), [PR #70](https://github.com/martlj/napari-clemreg/pull/70)).
...
- Package split, phase 1: FM and EM segmentation and ROI masking are napari-free, with new core functions `segment_fm` and `segment_em` behind the existing adapters. A new import-boundary test checks that the core modules import with napari and Qt blocked; the four not yet converted are listed as expected failures. The old functions still accept napari layers, so copies of the published batch notebook keep working. Results are unchanged: the pipeline output baseline passes as-is ([PR #71](https://github.com/martlj/napari-clemreg/pull/71)).
